...
const validateMain = (exports.validateMain = async ({
  ...
  _validateMainExtension(indexMetadata)
})
...
const _validateMainExtension = indexMetadata => {
  ...
  assert(
    openFormats.includes(ext.toLowerCase()),
    ValidationError,
    `Main file should be an open format file. Received extension: ${ext}`,
    'main_extension',
    'p2pcommons.main'
  )
}
 ...
 * @param {[Boolean]} params._flat - internal option - indicates whether the metadata is flattened
 */
exports.validateOnFollow = ({ followedIndexMetadata, _flat = true }) => {
  if (_flat) {
