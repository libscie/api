...
exports.validate = (indexMetadata, dbMetadata, key, p2pcommonsDir, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
    _validateMainExists(indexMetadata, key, p2pcommonsDir, unflatten)
    validateDraft(indexMetadata, dbMetadata, key, unflatten)
}

const validateDraft = (exports.validateDraft = (indexMetadata, dbMetadata, key, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
    validateTitle(indexMetadata, unflatten)
    validateDescription(indexMetadata, unflatten)
    validateUrl(indexMetadata, key, unflatten)
    ...
    validateMainDraft(indexMetadata, unflatten)
    validateAvatar(indexMetadata, unflatten)
    validateAuthors(indexMetadata, unflatten)
    validateParents(indexMetadata, dbMetadata, key, unflatten)
    validateFollows(indexMetadata, key, unflatten)
...
exports.validateBeforeInit = (indexMetadata, unflatten = true) => {
    ...
    if (indexMetadata.p2pcommons.parents !== undefined) {
        validateParents(indexMetadata, { version: 0 }, "", unflatten)
    }
    if (indexMetadata.p2pcommons.follows !== undefined) {
        validateFollows(indexMetadata, "", unflatten)
    }
...
const validateUrl = (exports.validateUrl = (indexMetadata, key, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
...
const validateP2pcommons = (exports.validateP2pcommons = (indexMetadata, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
...
exports.validateMain = (indexMetadata, key, p2pcommonsDir, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
    validateMainDraft(indexMetadata, unflatten)
...
const validateMainDraft = (exports.validateMainDraft = (indexMetadata, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
...
const _validateMainExists = (indexMetadata, key, p2pcommonsDir, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
    assert(
...
const _validateAuthorsFormat = (indexMetadata) => {
    const regex = /^(hyper:\/\/)?[A-Za-z0-9]{64}$/ //TEMP
    if (indexMetadata.p2pcommons.authors !== undefined) {
...
const validateParents = (exports.validateParents = (indexMetadata, dbMetadata, key, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
...
const _validateParentsFormat = (indexMetadata) => {
    const regex = /^(hyper:\/\/)?[A-Za-z0-9]{64}(\+\d+)$/ //TEMP
    if (indexMetadata.p2pcommons.parents !== undefined) {
...
exports.validateParentsOnUpdate = async (indexMetadata, p2pcommons, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
    ...
    if (indexMetadata.p2pcommons.parents !== undefined) {
        for (const parentKey of indexMetadata.p2pcommons.parents) {
            ...
            for (const authorKey of parent.authors) {
                ...
                if (author.contents.includes(parentKey) || author.contents.includes(`hyper://${parentKey}`)) {
                    isRegistered = true
                    break
                }
            }
...
const validateFollows = (exports.validateFollows = (indexMetadata, key, unflatten = true) => {
    if (unflatten) {
        indexMetadata = _unflattenIndexMetadata(indexMetadata)
    }
...
exports.validateOnRegister = (contentIndexMetadata, profileKey, unflatten = true) => {
    if (unflatten) {
...
const _validateOnRegisterAuthorsContainsProfileKey = (contentIndexMetadata, profileKey) => {
    assert(
        contentIndexMetadata.p2pcommons.authors.includes(profileKey)
        || contentIndexMetadata.p2pcommons.authors.includes(`hyper://${profileKey}`), //TEMP
        ValidationError,
...
exports.validateOnFollow = (followedIndexMetadata, unflatten = true) => {
    if (unflatten) {
...
const _unflattenIndexMetadata = indexMetadata => {
    let unflattened = Object.assign({}, indexMetadata)
