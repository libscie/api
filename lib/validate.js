 ...
 * @param {[Boolean]} unflatten - set false to validate non-flattened metadata (i.e. directly read from index.json)
 */
const validate = (exports.validate = async (indexMetadata, dbMetadata, key, p2pcommonsDir, unflatten = true) => {
  if (unflatten) {
  ...
  await validateMain(indexMetadata, key, p2pcommonsDir, unflatten)
})

/**
 * Validates all present data against the p2pcommons specs.
 * Can be used for validating unfinished modules.
 * Empty p2pcommons.main will not be validated
 *
 * @public
 * @async
 * @throws ValidationError
 ...
 * @param {[Boolean]} unflatten - set false to validate non-flattened metadata (i.e. directly read from index.json)
 */
exports.validatePartial = async (indexMetadata, dbMetadata = { version: 0 }, key = '', p2pcommonsDir = '', unflatten = true) => {
  if (unflatten) {
    indexMetadata = _unflattenIndexMetadata(indexMetadata)
  }
 ...
 * @param {[Boolean]} unflatten - set false to validate non-flattened metadata (i.e. directly read from index.json)
 */
const validateMain = (exports.validateMain = async (indexMetadata, key, p2pcommonsDir, unflatten = true) => {
  if (unflatten) {
    indexMetadata = _unflattenIndexMetadata(indexMetadata)
  }
  ...
  await _validateMainExists(indexMetadata, key, p2pcommonsDir, unflatten)
})
...
const _validateMainExists = async (indexMetadata, key, p2pcommonsDir, unflatten = true) => {
  if (unflatten) {
    indexMetadata = _unflattenIndexMetadata(indexMetadata)
  }
  assert(
 ...
 * @param {[Boolean]} unflatten - set false to validate non-flattened metadata (i.e. directly read from index.json)
 */
exports.validateOnRegister = async (contentIndexMetadata, contentDbMetadata, contentKey, profileIndexMetadata, profileDbMetadata, profileKey, p2pcommonsDir, unflatten = true) => {
  if (unflatten) {
  ...
  await validate(contentIndexMetadata, contentDbMetadata, contentKey, p2pcommonsDir, unflatten)
  await validate(profileIndexMetadata, profileDbMetadata, profileKey, p2pcommonsDir, unflatten)
}
