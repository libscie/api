...
const replicateFeed = (store, feed) => {
  store.emit('feed', feed)

  const { stream, opts } = store.replicationStreams
  if (stream) {
    debug('replicating corestore')
...
const open = (
  ...
  return { drive: Hyperdrive(driveStorage, key, hyperdriveOpts), driveStorage }
}
