...
const importFiles = async (archive, src = './', opts = {}) => {
  ...
  const ignore = (name, st) => {
    ...
    debug(`ignore ${name} : ${dIgnore(name)}`)
    return dIgnore(name)
  }
...
const downloadFiles = (archive, dst, opts = {}) => {
  ...
  const dIgnore = datIgnore(dst)

  const ignoreDirs = !(opts.ignoreDirs === false)
  const ignore = (name, st) => {
    if (ignoreDirs && st && st.isDirectory()) return true
    debug(`downloadFiles: ignore ${name} : ${dIgnore(name)}`)
    return dIgnore(name)
  }
  const finalOpts = {
    watch: false,
 ...
 * }
 */
const open = async (store, key, hyperdriveOpts = {}) => {
