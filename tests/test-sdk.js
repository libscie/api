...
module.exports = async dht => {
  ...
  test('check lastModified on ready (refreshMTimes)', async t => {
    ...
    await writeFile(
      join(dir, contentPath, 'index.json'),
  ...
  test('sdk re-start', async t => {
    ...
    const p2p = createSdk({ swarm: true, persist: true, dht })

    const p2p2 = createSdk({ swarm: true, persist: true, dht })

    const p2p3 = createSdk({ swarm: true, persist: true, dht })
